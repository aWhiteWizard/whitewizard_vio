...
void PubImageData()
{
	...
	while (std::getline(fsImage, sImage_line) && !sImage_line.empty())
	{
		...
		pSystem->PubImgData(dStampNSec / 1e9, img);
...
int main(int argc, char **argv)
{
	if(argc != 3)
	{
		cerr << "./test_euroc PATH_TO_FOLDER/MH-05/mav0 PATH_TO_CONFIG/config \n" 
			<< "For example: ./test_euroc /home/whitewizard/4_dataset/MH_05_difficult/mav0/ ../config/"<< endl;
		return -1;
	}
	sData_path = argv[1];
	sConfig_path = argv[2];
	...
	std::thread thd_BackEnd(&System::BackEnd, pSystem);
