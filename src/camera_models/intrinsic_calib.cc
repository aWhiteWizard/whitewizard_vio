...
int main(int argc, char** argv)
{
    ...
    camera_models::Camera::ModelType modelType;
    if (boost::iequals(cameraModel, "kannala-brandt"))
    {
        modelType = camera_models::Camera::KANNALA_BRANDT;
    }
    else if (boost::iequals(cameraModel, "mei"))
    {
        modelType = camera_models::Camera::MEI;
    }
    else if (boost::iequals(cameraModel, "pinhole"))
    {
        modelType = camera_models::Camera::PINHOLE;
    }
    else if (boost::iequals(cameraModel, "scaramuzza"))
    {
        modelType = camera_models::Camera::SCARAMUZZA;
    }
    else
    {
    ...
    switch (modelType)
    {
    case camera_models::Camera::KANNALA_BRANDT:
        std::cout << "# INFO: Camera model: Kannala-Brandt" << std::endl;
        break;
    case camera_models::Camera::MEI:
        std::cout << "# INFO: Camera model: Mei" << std::endl;
        break;
    case camera_models::Camera::PINHOLE:
        std::cout << "# INFO: Camera model: Pinhole" << std::endl;
        break;
    case camera_models::Camera::SCARAMUZZA:
    ...
    camera_models::CameraCalibration calibration(modelType, cameraName, frameSize, boardSize, squareSize);
    ...
    for (size_t i = 0; i < imageFilenames.size(); ++i)
    {
        ...
        camera_models::Chessboard chessboard(boardSize, image);
    ...
    double startTime = camera_models::timeInSeconds();
    ...
    if (verbose)
    {
        std::cout << "# INFO: Calibration took a total time of "
                  << std::fixed << std::setprecision(3) << camera_models::timeInSeconds() - startTime
                  << " sec.\n";
    }
