                                          ...
                                          int flags)
{
    ...
    for (int k = 0; k < 6; ++k)
    {
        for (int dilations = minDilations; dilations <= maxDilations; ++dilations)
        {
            ...
            cv::Mat kernel1 = cv::getStructuringElement(MorphShapes::MORPH_CROSS, cv::Size(3,3), cv::Point(1,1));
            cv::Mat kernel2 = cv::getStructuringElement(MorphShapes::MORPH_RECT, cv::Size(3,3), cv::Point(1,1));
