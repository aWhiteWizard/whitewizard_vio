//whitewizard created on Sep 11th
#include "System.h"
...
System::System(string sConfig_files_):bStart_backend(true)
{
    string sConfig_file = sConfig_files_ + "euroc_config.yaml";
...
void System::PubImgData(double dStampSec, Mat &img)
{
    if (!init_feature)
    {
        cout << "1 PubImgData skip the first detected feature, which doesn't contain optical flow speed" << endl;
        init_feature = 1;
        return;
    }
    ...
    if (first_image_flag)
    {
        cout << "2 PubImgData first_image_flag" << endl;
    ...
    {
        cerr << "3 PubImgData image discontinue! reset the feature tracker!" << endl;
    ...
    timecheck t_r;
    // cout << "3 PubImgData t : " << dStampSec << endl;
    ...
    {
        pub_count++;
        shared_ptr<IMG_MESSAGE> feature_points(new IMG_MESSAGE());
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            for (unsigned int j = 0; j < ids.size(); j++)
            {
                if (trackerData[i].track_cnt[j] > 1)
                {
                    ...
                    feature_points->point_id.push_back(p_id * NUM_OF_CAM + i);
                    feature_points->point_u.push_back(cur_pts[j].x);
                    feature_points->point_v.push_back(cur_pts[j].y);
                    feature_points->point_vel_x.push_back(pts_velocity[j].x);
                    feature_points->point_vel_y.push_back(pts_velocity[j].y);
                }
            }
            //}
...
void System::bPubImgData(double dStampSec, const vector<cv::Point2f> &FeaturePoints)
{
    if (!init_feature)
    {
        cout << "1 PubImgData skip the first detected feature, which doesn't contain optical flow speed" << endl;
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            for (unsigned int j = 0; j < FeaturePoints.size(); j++)
            {
                ...
                feature_points->point_u.push_back(pixel_point.x); // 像素坐标
                feature_points->point_v.push_back(pixel_point.y);
                feature_points->point_vel_x.push_back(0);
                feature_points->point_vel_y.push_back(0);
//                }
            }
            //}
            // skip the first image; since no optical speed on frist image
...
{
    shared_ptr<IMU_MESSAGE> imu_msg(new IMU_MESSAGE());
	imu_msg->header = dStampSec;
	imu_msg->linear_acc = vAcc;
	imu_msg->angular_vec = vGyr;
    ...
    //     << " acc: " << imu_msg->linear_acc.transpose()
    //     << " gyr: " << imu_msg->angular_vec.transpose() << endl;
    m_buf.lock();
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                if (t <= img_t)
                {
                    ...
                    assert(dt >= 0);
                    current_time = t;
                    dx = imu_msg->linear_acc.x();
                    dy = imu_msg->linear_acc.y();
                    dz = imu_msg->linear_acc.z();
                    rx = imu_msg->angular_vec.x();
                    ry = imu_msg->angular_vec.y();
                    rz = imu_msg->angular_vec.z();
                ...
                else
                {
                    ...
                    dx = w1 * dx + w2 * imu_msg->linear_acc.x();
                    dy = w1 * dy + w2 * imu_msg->linear_acc.y();
                    dz = w1 * dz + w2 * imu_msg->linear_acc.z();
                    rx = w1 * rx + w2 * imu_msg->angular_vec.x();
                    ry = w1 * ry + w2 * imu_msg->angular_vec.y();
                    rz = w1 * rz + w2 * imu_msg->angular_vec.z();
            ...
            {
                int v = img_msg->point_id[i] + 0.5;
                int feature_id = v / NUM_OF_CAM;
                int camera_id = v % NUM_OF_CAM;
                double x = img_msg->points[i].x();
                double y = img_msg->points[i].y();
                double z = img_msg->points[i].z();
                double p_u = img_msg->point_u[i];
                double p_v = img_msg->point_v[i];
                double velocity_x = img_msg->point_vel_x[i];
                double velocity_y = img_msg->point_vel_y[i];
                assert(z == 1);
                Eigen::Matrix<double, 7, 1> xyz_uv_velocity;
                xyz_uv_velocity << x, y, z, p_u, p_v, velocity_x, velocity_y;
                image[feature_id].emplace_back(camera_id, xyz_uv_velocity);
            }
            timecheck t_processImage;
            ...
            if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
            {
                ...
                // cout << "1 BackEnd processImage dt: " << fixed << t_processImage.check_time << " stamp: " <<  dStamp << " p_wi: " << p_wi.transpose() << endl;
